...
int GMEParameter::initial() {
  ...
  cv::compare(motionVector, tmp1.at<double>(0) + tmp2.at<double>(0), ltFlag,
              cv::CMP_LT);
  cv::compare(motionVector, tmp1.at<double>(0) - tmp2.at<double>(0), gtFlag,
              cv::CMP_GT);
...
bool GMEParameter6::checkIfStop() {
  ...
  cv::pow(parameter - preParameter, 2, tmp1);
  parDiff1 = (tmp1.at<float>(2) + tmp1.at<float>(5)) * 0.5;
  parDiff2 = (tmp1.at<float>(0) + tmp1.at<float>(1) + tmp1.at<float>(3) +
              tmp1.at<float>(4)) *
             0.25;
...
void GMEParameter6::calculateParameter() {
  ...
  cv::addWeighted(GME->xPos, parameter.at<float>(0), GME->yPos,
                  parameter.at<float>(1), parameter.at<float>(2), alpha);
  cv::addWeighted(GME->xPos, parameter.at<float>(3), GME->yPos,
                  parameter.at<float>(4), parameter.at<float>(5), beta);
  ...
  cv::addWeighted(GME->xPos, parameter.at<float>(0), GME->yPos,
                  parameter.at<float>(1), parameter.at<float>(2), xCalPos);
  cv::addWeighted(GME->xPos, parameter.at<float>(3), GME->yPos,
                  parameter.at<float>(4), parameter.at<float>(5), yCalPos);
}